...
import { ArrowRight, Mail, Lock, User, Eye, EyeOff, AtSign, CheckCircle2, AlertCircle } from "lucide-react";
...
const Auth = () => {
  ...
  const handleSignup = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.post("/register/simple/", {
      ...
      // Use the login function to initialize the app state
      const result = await login(signupForm.username.trim().toLowerCase(), signupForm.password);
      if (result.success) {
        toast.success("Account created! Welcome to Baarter 🎊");
        nav("/app/feed");
      }
    } catch (err) {
      ...
      if (data.password) setError("password", data.password);
      if (data.name) setError("name", data.name);
      if (!Object.keys(data).length) toast.error("Signup failed. Please try again.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      afterAuth(res.data.access, res.data.refresh);
      // Re-use login but via token directly — initialize app state
          ...
          <div className="w-full max-w-md relative py-8">
            ...
            <div className="backdrop-blur bg-white/[0.03] border border-white/8 rounded-full p-1 flex mb-8 w-fit">
              {["signup", "login"].map(tab => (
                <button
                  key={tab}
                  onClick={() => { setMode(tab); clearErrors(); }}
