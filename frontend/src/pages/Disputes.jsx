...
const Disputes = () => {
  const { disputes, users, setDisputes } = useApp();
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({ reason: "", detail: "" });

  const file = () => {
    if (!form.reason) { toast.error("Pick a reason"); return; }
    const newD = { id: `d_${Date.now()}`, swap: "-", against: "u_kai", status: "open", ...form, opened: "just now" };
              ...
              <div key={d.id} className="nb-card p-4 bg-[var(--surface)]" data-testid={`dispute-${d.id}`}>
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <div className="font-display text-xl">Against {against?.name}</div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setOpen(false)}>
          <div onClick={(e) => e.stopPropagation()} className="nb-card p-6 bg-[var(--surface)] w-full max-w-md" data-testid="dispute-modal">
            ...
            <div className="flex gap-2">
              <NbButton variant="light" onClick={() => setOpen(false)} className="flex-1">Cancel</NbButton>
              <NbButton onClick={file} className="flex-1" data-testid="dispute-submit">File dispute <ArrowRight size={14} strokeWidth={3} /></NbButton>
            </div>
          </div>
        </div>
      )}
